...
export function LanyardCard({
  ...
  return (
    <div className={`${className}`} style={{ width, height }}>
      <Canvas camera={{ position: [0, 0, 13], fov: 25 }} onCreated={({camera}) => {
        camera.lookAt(0, 0, 0)
      }}>
...
// Legacy EventBadge component for backward compatibility
...
const EventBadge = ({ 
  ...
  if (!mounted) {
    return (
      <div 
        className={`bg-gradient-to-br from-pink-500 to-pink-600 rounded-xl shadow-2xl flex items-center justify-center ${className}`}
        style={{ width, height }}
        aria-hidden="true"
      >
        <div className="text-white text-center">
          <div className="text-3xl font-bold tracking-wide">ANH HOANG</div>
          <div className="text-sm tracking-widest opacity-90 mt-1">PORTFOLIO</div>
        </div>
      </div>
    )
  }
